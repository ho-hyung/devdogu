...
export default function TimestampClient() {
  ...
  const tsToDate = () => {
    ...
    if (isNaN(num)) { setDateResult('유효하지 않은 숫자입니다.'); return; }
    // 자동 감지: 13자리면 밀리초, 10자리면 초
    const ms = tsInput.length >= 13 ? num : num * 1000;
    const d = new Date(ms);
    if (isNaN(d.getTime())) { setDateResult('유효하지 않은 타임스탬프입니다.'); return; }
    setDateResult(
  ...
  const dateToTs = () => {
    ...
    if (isNaN(d.getTime())) { setTsResult('유효하지 않은 날짜 형식입니다.'); return; }
    setTsResult(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl text-center">
        <p className="text-xs text-[var(--color-text-secondary)] uppercase tracking-wider mb-2">현재 Unix 타임스탬프</p>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl space-y-3">
          ...
          />
          <button onClick={tsToDate} className="btn-primary w-full">변환</button>
          {dateResult && (
        ...
        <div className="p-5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl space-y-3">
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg font-mono text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30"
          />
          <button onClick={dateToTs} className="btn-primary w-full">변환</button>
          {tsResult && (
            <pre className="p-3 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg font-mono text-xs whitespace-pre-wrap">
              {tsResult}
            </pre>
