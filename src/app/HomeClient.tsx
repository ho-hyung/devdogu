...
export default function HomeClient({ locale }: HomeClientProps) {
  ...
  const filtered = tools.filter((tool) => {
    ...
    return (
      ...
      tool.name[locale].toLowerCase().includes(query) ||
      tool.description[locale].toLowerCase().includes(query) ||
    ...
    <>
      ...
      {(query || activeCategory) && (
        <p className="text-center text-xs text-[var(--color-text-secondary)] mb-6">
          {dict.home.toolCount.replace('{count}', String(filtered.length))}
          {query && ` (${dict.home.searchLabel.replace('{query}', search)})`}
        </p>
      )}
      ...
      {showGrouped && favHydrated && favoriteTools.length > 0 && (
        <section className="mb-10">
          ...
          <ToolGrid tools={favoriteTools} locale={locale} onToggleFavorite={toggleFavorite} isFavorite={isFavorite} toolHref={toolHref} dict={dict} />
        </section>
      )}
      ...
      {showGrouped && recentHydrated && recentTools.length > 0 && (
        <section className="mb-10">
          ...
          <ToolGrid tools={recentTools} locale={locale} onToggleFavorite={toggleFavorite} isFavorite={isFavorite} toolHref={toolHref} dict={dict} />
        </section>
      )}
      ...
      <section>
        {showGrouped ? (
          <div className="space-y-10">
            {groupedByCategory.map((group) => (
              <div key={group.key}>
                ...
                <ToolGrid tools={group.tools} locale={locale} onToggleFavorite={toggleFavorite} isFavorite={isFavorite} toolHref={toolHref} dict={dict} />
              </div>
            ))}
          </div>
        ) : (
          <>
            {filtered.length > 0 ? (
              <ToolGrid tools={filtered} locale={locale} onToggleFavorite={toggleFavorite} isFavorite={isFavorite} toolHref={toolHref} dict={dict} />
            ) : (
              <div className="text-center py-16">
                <p className="text-[var(--color-text-secondary)] mb-2">
                  {dict.home.noResultsFor.replace('{query}', search)}
                </p>
                <button
          ...
          <div className="mt-4">
            <div className="tool-card opacity-50 cursor-default">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="font-semibold text-[15px] mb-1">{dict.home.moreTools}</h2>
                  <p className="text-sm text-[var(--color-text-secondary)]">
                    {dict.home.moreToolsDesc}
                  </p>
                </div>
              </div>
            </div>
...
function ToolGrid({ tools, locale, onToggleFavorite, isFavorite, toolHref, dict }: ToolGridProps) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {tools.map((tool) => (
        <div key={tool.id} className="tool-card group relative">
          <Link href={toolHref(tool.href)} className="flex items-start gap-4">
          ...
          {onToggleFavorite && isFavorite && (
            <button
              ...
              aria-label={isFavorite(tool.id) ? dict.home.removeFavorite : dict.home.addFavorite}
            >
              <svg
