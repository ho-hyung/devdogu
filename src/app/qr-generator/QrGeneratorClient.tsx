...
export default function QrGeneratorClient() {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      setError('QR코드를 생성할 수 없습니다. 입력 데이터가 너무 길 수 있습니다.');
      setDataUrl('');
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">텍스트 또는 URL</label>
            <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="QR코드로 변환할 텍스트나 URL을 입력하세요..." className="input-area min-h-[120px]" spellCheck={false} />
          </div>
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <label className="text-sm text-[var(--color-text-secondary)]">크기</label>
            ...
            <div className="flex items-center gap-2">
              <label className="text-sm text-[var(--color-text-secondary)]">오류 복원</label>
        ...
        <div className="flex flex-col items-center gap-4">
          {dataUrl ? (
            <>
              ...
              <button onClick={handleDownload} className="btn-primary">PNG 다운로드</button>
            </>
          ) : (
            <div className="w-64 h-64 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl flex items-center justify-center text-[var(--color-text-secondary)] text-sm">QR코드가 여기에 표시됩니다</div>
          )}
        </div>
      </div>
      {error && (
